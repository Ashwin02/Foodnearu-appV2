...
export class OrderOnlinePage  implements OnInit{
  ...
  {
    let resUrl;
    resUrl = `${this.ROOT_URL_RES}${hotel.Restaurant.id}/${hotel.Restaurant.img}_small_thumb.${hotel.Restaurant.img_ext}`;
    return resUrl;
  }
  ...
  ngOnInit(){
    ...
    console.log("LAt n long "+ this.lat);
      ...
      this.dataService.getResNearMe(this.areaCode).subscribe((res) => 
        {
          ...
          // this.isOpen(this.hotelsNearMe);


          // this.resTimings = this.hotelsNearMe.filter((restaurant)=>{

          //   return restaurant.Restaurant_timings;
          // })
          // console.log("Res "+JSON.stringify(this.hotelsNearMe))
          // console.log(this.resTimings);
        });
    }
